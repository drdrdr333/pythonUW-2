...
def load_users(_obj, user_collection=users.UserCollection()):
    '''
    ...
    '''
    # print(_obj, dir(_obj), type(_obj))
    for index, row in _obj.iterrows():
        t = user_collection.add_user(
            row['USER_ID'],
            row['EMAIL'],
            row['NAME'],
            row['LASTNAME']
...
def save_users(filename, user_collection):
    ...
    try:
        ...
        if isinstance(filename, str):
            ...
            logging.info("%s users added to %s", count, filename)
            return True
        ...
        if isinstance(filename, str):
            ...
            with open(filename, 'w+', newline='') as _f:
                ...
                for key,val in status_collection.database.items():
                    ...
                    print(f"Row added to {filename}.")
                    count += 1
            logging.info("%s statuses added to %s", count, filename)
            return True
    except IndexError:
        return False
...
def add_status(user_id, status_id, status_text, status_collection):
    ...
    if not isinstance(user_id, str):
        raise TypeError('User id field should be a string.')
    if not isinstance(status_collection, user_status.UserStatusCollection):
        raise TypeError("Invalid collection. Please supply a status collection as final arg.")
    stats = status_collection.add_status(status_id, user_id, status_text)
    return stats
